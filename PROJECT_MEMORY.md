...
- **總收錄規模**：**40,061 筆**外文譯名記錄（橫跨 26 個子工作表 A ~ Z）
- **欄位格式**：
...
> ⚠️ **重要經驗**：Google Sheets 在「建立副本」或「匯入工作表」時會重新隨機生成 GID。若 GID 錯誤，Google GViz API 會**自動退回載入第一個分頁（A 分頁）**。以下為實測 100% 驗證之最新真實 GID 陣列：

| 分頁代碼 | 工作表 GID | 資料筆數 | 首筆資料範例（英文 / 中文 / 號碼） |
| :---: | :---: | :---: | :--- |
| **A** | `1218758949` | 2,332 | `Aagesen` / `阿格森` / `7144` |
| **B** | `1972503810` | 4,272 | `Baader` / `巴德爾` / `7721` |
| **C** | `2012113025` | 1,321 | `Christenson` / `克里坦遜` / `4064` |
| **D** | `1724523776` | 2,247 | `D'Adamo` / `德阿達摩` / `2473` |
| **E** | `788033022` | 1,028 | `Eachard` / `厄傑德` / `7122` |
| **F** | `1600415547` | 1,540 | `Fabbri` / `法布里` / `3446` |
| **G** | `862254614` | 2,410 | `Gaardar` / `賈德` / `1024` |
| **H** | `1097899205` | 2,471 | `Ha` / `哈` / `6806` |
| **I** | `1091090828` | 413 | `Iacoboni` / `亞科波尼` / `1023` |
| **J** | `1899163508` | 696 | `Jaabari` / `賈巴瑞` / `1071` |
| **K** | `1059918597` | 2,292 | `Ka` / `卡` / `2123` |
| **L** | `1751943907` | 2,233 | `L'Allemand` / `拉爾曼` / `5016` |
| **M** | `425283326` | 3,426 | `Ma` / `馬` / `7132` |
| **N** | `1907320444` | 864 | `Naaman` / `納曼` / `2460` |
| **O** | `463957896` | 736 | `Oades` / `歐狄斯` / `7744` |
| **P** | `1896294959` | 2,142 | `Pa Paioannon` / `巴巴亞農` / `7771` |
| **Q** | `666753901` | 124 | `Qabus` / `卡巴斯` / `2174` |
| **R** | `1648537175` | 1,889 | `Ra Anan` / `拉安南` / `5034` |
| **S** | `1670759665` | 4,092 | `Saade` / `薩德` / `4424` |
| **T** | `1427337914` | 255 | `Trued` / `特魯德` / `2422` |
| **U** | `1833395285` | 242 | `Ubac` / `烏巴克` / `2774` |
| **V** | `654723940` | 926 | `Vaca` / `瓦加` / `1046` |
| **W** | `2066089904` | 1,288 | `Waage` / `佛格` / `2547` |
| **X** | `1275336458` | 18 | `Xaio` / `查艾奧` / `4042` |
| **Y** | `1520584080` | 270 | `Yaacob` / `雅可布` / `7014` |
| **Z** | `835318692` | 534 | `Zababakhin` / `札巴巴金` / `4277` |
...
flowchart TD
    ...
    E --> F[聚合 40,061 筆資料至 allAggregatedRows & tabDataMap]
...
- **解決方案**：透過動態 `<script>` 標籤注入呼叫 Google Visualization JSONP 端點：
  `https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq?tqx=responseHandler:${callbackName}&gid=${gid}`
...
3. **📊 4 大即時統計指標卡片**：
   - 總資料庫筆數（40,061 筆）、目前符合筆數、當前分頁筆數、已載入分頁數（26/26）。
4. **🔄 雙檢視模式**：
